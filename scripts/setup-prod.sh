...
# Script para setup completo do ambiente PROD no Google Cloud - Backoffice
...
# Configurações
SERVICE_ACCOUNT_NAME="github-actions-backoffice-prod"
SERVICE_ACCOUNT_DISPLAY="GitHub Actions Backoffice Prod"
REPO_NAME="agiliza-backoffice"
REGION="us-central1"
...
if [ -z "$1" ]; then
    ...
    echo -e "${YELLOW}Exemplo: ./scripts/setup-prod.sh agiliza-prod${NC}"
    exit 1
fi

PROJECT_ID=$1

echo -e "${BLUE}🚀 Setup do Ambiente PROD - Agiliza Backoffice${NC}"
echo -e "${===============================================${NC}"
...
if gcloud iam service-accounts describe $SERVICE_ACCOUNT_NAME@$PROJECT_ID.iam.gserviceaccount.com &>/dev/null; then
    ...
    gcloud iam service-accounts create $SERVICE_ACCOUNT_NAME \
        --description="Conta de serviço para GitHub Actions - Backoffice Produção" \
...
if gcloud artifacts repositories describe $REPO_NAME --location=$REGION &>/dev/null; then
    ...
    gcloud artifacts repositories create $REPO_NAME \
        ...
        --location=$REGION \
        --description="Repositório Docker para Agiliza Backoffice - Produção"
    echo -e "${GREEN}✅ Repositório criado${NC}"
fi

# Gerar chave JSON
echo -e "${BLUE}🔑 Gerando chave JSON...${NC}"
KEY_FILE="github-actions-backoffice-prod-key.json"
...
echo -e "1. Copie o conteúdo do arquivo '${KEY_FILE}' para o secret 'GCP_SA_KEY_BACKOFFICE_PROD' no GitHub"
echo -e "2. Configure os outros secrets no GitHub:"
echo -e "   - GCP_PROJECT_ID_BACKOFFICE_PROD=${PROJECT_ID}"
echo -e "   - GCP_REGION_BACKOFFICE_PROD=${REGION}"
echo -e "   - FRONTEND_SERVICE_NAME_PROD=agiliza-backoffice-prod"
echo -e "   - NEXT_TELEMETRY_DISABLED=1"
echo -e "   - NEXT_PUBLIC_API_URL_PROD=https://agiliza-api-nafwgg52oq-uc.a.run.app"
echo -e "   - NEXT_PUBLIC_WS_URL_PROD=wss://agiliza-api-nafwgg52oq-uc.a.run.app"
echo -e "   - NEXT_PUBLIC_APP_NAME_PROD=Agiliza Backoffice"
echo ""
